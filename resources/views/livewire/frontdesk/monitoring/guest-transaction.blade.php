  ...
  <div class="lg:grid  lg:grid-cols-12 lg:gap-8 ">
            ...
            <div class="bg-gradient-to-br shadow-md from-gray-300 via-gray-200 to-gray-100 mt-2 rounded-lg p-3">
              <div class="header">
                ...
                <div class="mt-2 border-b border-gray-300">
                  ...
                  <div class="font-bold text-gray-700" wire:ignore>
                    @if($guest->checkinDetail?->check_out_at)
                      <div x-data="countdown('{{ Carbon\Carbon::parse($guest->checkinDetail->check_out_at)->toIso8601String() }}')" x-init="start()" x-on:remove.window="destroy()">
                        <template x-if="state === 'expired'">
                          <span class="text-sm font-medium text-red-700">Time Expired!</span>
                        </template>
                        <template x-if="state === 'grace'">
                          <div class="text-yellow-600">
                            <span class="text-sm font-medium">Grace Period:</span>
    ...
    <main class="lg:col-span-9 xl:col-span-7">
      <div class="grid grid-cols-6 gap-x-2 border rounded-lg p-4 mb-4">
        ...
        <x-button :disabled="$grace_end < Carbon\Carbon::now()" label="Damage Charges" sm blue right-icon="arrow-top-right-on-square" href="{{ route('frontdesk.damage-charges', ['id' => $guest_id]) }}" />
        <x-button :disabled="$grace_end < Carbon\Carbon::now()" label="Amenities" sm blue right-icon="arrow-top-right-on-square" href="{{ route('frontdesk.amenities', ['id' => $guest_id]) }}" />
        <x-button :disabled="$grace_end < Carbon\Carbon::now()" label="Food and Beverages" sm blue right-icon="arrow-top-right-on-square"
          href="{{ route('frontdesk.food-beverages', ['id' => $guest_id]) }}" />
        <x-button :disabled="$grace_end < Carbon\Carbon::now()" label="Deposits" sm blue right-icon="arrow-top-right-on-square" href="{{ route('frontdesk.guest-deposit', ['id' => $guest_id]) }}" />
      </div>
