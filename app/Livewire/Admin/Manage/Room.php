            ...
            Action::make('manage_rates')
                ...
                ->form(function ($record) {
                    ...
                    foreach ($stayingHours as $sh) {
                        $existingRate = Rate::where('room_id', $record->id)
                            ->where('staying_hour_id', $sh->id)
                            ->first();
                        $fields[] = TextInput::make('rate_' . $sh->id)
                            ->label($sh->number . ' Hours')
                            ->numeric()
                            ->prefix('₱')
                            ->default($existingRate?->amount ?? 0);
                    }
                    return [Grid::make(2)->schema($fields)];
                })
                ->action(function ($record, $data) {
                    $branchId = $record->branch_id;
                    $stayingHours = StayingHour::where('branch_id', $branchId)->get();
                    foreach ($stayingHours as $sh) {
                        $amount = $data['rate_' . $sh->id] ?? 0;
                        Rate::updateOrCreate(
                            [
                                'room_id' => $record->id,
